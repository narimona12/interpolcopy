    ...
    <>
      <div id="root">
        ...
        <div className="fixed z-[-1] top-0 left-0 w-full h-[100vh] overflow-hidden bg-[#2A4B8A]">
          <img src="/assets/m.a4159446.svg" alt="bg_map" className="w-[150%] sm:w-full h-full object-cover sm:object-contain opacity-40 absolute top-0 left-[-30%] sm:left-0 object-left" />
        </div>
        ...
        <div className="flex items-center z-[99] justify-start flex-col w-full min-h-[100vh] px-4 sm:py-5 py-[20px] mx-auto">
          ...
          <div className="flex max-w-[425px] mx-auto mb-[20px] items-start gap-3 justify-start w-full flex-col">
            <a href="/images/verified_doctors/client_1/IMG_5061.jpg" target="_blank" rel="noopener noreferrer" className="block w-full">
              <button className="align-middle select-none font-sans font-bold text-black text-center uppercase transition-all disabled:opacity-50 disabled:shadow-none disabled:pointer-events-none px-6 bg-white shadow-md hover:shadow-lg focus:shadow-none active:shadow-none block w-full text-[16px] py-[10px] rounded" type="button">
                {t.btn_id}
              </button>
            </a>
            <a href="/images/verified_doctors/client_1/IMG_5063.jpg" target="_blank" rel="noopener noreferrer" className="block w-full">
              <button className="align-middle select-none font-sans font-bold text-black text-center uppercase transition-all disabled:opacity-50 disabled:shadow-none disabled:pointer-events-none px-6 bg-white shadow-md hover:shadow-lg focus:shadow-none active:shadow-none block w-full text-[16px] py-[10px] rounded" type="button">
                {t.btn_license}
              </button>
            </a>
            <button onClick={() => setIsModalOpen(true)} className="align-middle select-none font-sans font-bold text-black text-center uppercase transition-all disabled:opacity-50 disabled:shadow-none disabled:pointer-events-none px-6 bg-white shadow-md hover:shadow-lg focus:shadow-none active:shadow-none block w-full text-[16px] py-[10px] rounded" type="button">
              {t.btn_cert}
            </button>
          </div>
        ...
        {/* Modal Block */}
        <div onClick={() => setIsModalOpen(false)} className={`items-center justify-center w-full h-[100vh] bg-[#14123990] z-[999] backdrop-blur-md ${isModalOpen ? 'flex' : 'hidden'} fixed top-0 left-0 p-6`}>
